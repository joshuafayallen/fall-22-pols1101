...
spender = spender_raw |> 
 ...
 spender_static =  ggplot(spender, aes(x = rank, fill = as.factor(name), group = as.factor(name))) +
  geom_tile(aes(y = money/2, height = money, width = .9), alpha = 0.5) +
  geom_text(aes(y = 0, label = paste(name,"")), vjust = 0.2, hjust = 1, size = 3.7) +
...
spender_static + facet_null() +
  ...
  labs(caption = 'Top Donors: {as.integer(closest_state)}')
