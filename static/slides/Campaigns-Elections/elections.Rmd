...
Lets think of a plate full of food that we would like to eat. Lets say we are looking at my plate. I have supposedly three option. Brocolli, Wings, and Beets. These can represent our political parties that are competing to get onto my plate. I would generally prefer not to eat beets not a huge beet guy just don't really like the taste. I would prefer to have a plate full of pizza. But I will eat the brocolli because I know it is good for me. Lets say the dreaded beet party has 35% support for making it on the plate this is an outcome I would not want. Lets say the pizza party has very few supporters lets say about 10%. But the brocolli party has 40% support. 

In a winner take all system people are generally going to vote for the brocolli party even if their true preference is the pizza party. While the pizza party perfectly represents our preferences the issue is that in our little district we have a lot of beet lovers. In order to not eat a beet I would take the broccili. Since there can only be one winner people are willing to settle for a more popular party that represents our preferences even it is option number 2 
...
???
Parties may try and contend around this space. In this case since the we have a winner take all as you can see the majority of people in the electorate may be distributed around the ideology scores of the various parties. As you can see 
...
pacman::p_load("tmap", "tidycensus", "sf", "scico", "leaflet")
georgia_map_data <- get_decennial(
  geography = "tract",
  state = "GA",
  variables = c(
    White = "P2_005N",
...
ggplot() +
  geom_sf(data = georgia_map_data ,aes(fill = percent), color = NA) +
  theme_void() +
...
???
Lets just pull all the data from the last decennial census and then plot it. Here I am just recyling some code from the ideology presentation but rather than just pulling specific tracts in and around atlanta here I am just pulling all the data from georgia. I just seperated it out by race. This is just more of a clarity decision on my part. Here we get a rough snapshot about some of the population distribution patterns in the state of georgia.
...
time_census = microbenchmark::microbenchmark(get_decennial(
  geography = "tract", state = "GA", variables = c(
    White = "P2_005N",
    Black = "P2_006N",
    Asian = "P2_008N",
    Hispanic = "P2_002N" ), summary_var = "P2_001N", year = 2020, geometry = TRUE
), times = 5)
...
The Total siuze of the data frame is about 1 second. It took 6 lines of code. I could very easily overlay any kind of censusus data I want. If you need election data overlayed. Easy. I would just need election returns. It would end up taking me longer to make a prettier map
...
These are essentially just retrospective assessments. 
...
This results in lots of people being sociotropic voters. Which economoy people consider is like a huge fight. What we do know is that people tend reward/punish the party in power by voting for or against the party in the white house. However this tends to be constrained to governors and repersentatives rather than all candidates Kessner and Warshaw(2020)
